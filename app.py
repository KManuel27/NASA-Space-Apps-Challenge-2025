...
#ORBITAL MECHANICS AND VISUALISATION CODE:
...
for step in range(max_frame+1):
    ...
    earth_traj = go.Scatter3d(x=earth_pos[:step+1,0], y=earth_pos[:step+1,1], z=earth_pos[:step+1,2],
                              mode='lines', line=dict(color='blue'), name='Earth trajectory')
    asteroid_traj = go.Scatter3d(x=asteroid_pos[:step+1,0], y=asteroid_pos[:step+1,1], z=asteroid_pos[:step+1,2],
